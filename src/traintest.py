...
def validate(audio_model, val_loader, args, epoch):
    ...
    audio_model.eval()
    conf_print= True if epoch == args.n_epochs else False
    end = time.time()
    A_predictions = []
    A_targets = []
    A_loss = []
    with torch.no_grad():
        ...
        loss = np.mean(A_loss)
        stats = calculate_stats(audio_output, target, conf_print)
...
def validate_ensemble(args, epoch):
    ...
    conf_print = False
    stats = calculate_stats(cum_predictions, target, conf_print)
    return stats
